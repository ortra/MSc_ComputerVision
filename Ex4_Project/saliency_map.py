...
def compute_gradient_saliency_maps(samples: torch.tensor,
    ...
    y_pred = model(samples) # forward pass
    y_pred_score_true_lables = y_pred[np.arange(true_labels.shape[0]),true_labels]
    y_pred_score_true_lables.backward(torch.ones_like(true_labels))
    samples_grad = samples.grad.data.abs()
    saliency, _ =  torch.max(samples_grad, dim = 1 )
