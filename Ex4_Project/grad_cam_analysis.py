...
def get_grad_cam_visualization(test_dataset: torch.utils.data.Dataset,
    ...
    # Link to reference code: https://github.com/jacobgil/pytorch-grad-cam#using-from-code-as-a-library
    target_layers = [model.conv3]

    # Creating an input tensor image for the model.
    input_tensor , input_label = next(iter(DataLoader(test_dataset,1,shuffle=True)))
    
    # Construct the CAM object once, and then re-use it on many images:
    is_cuda =  torch.cuda.is_available()
    cam = GradCAM(model=model, target_layers=target_layers, use_cuda=is_cuda)
    grayscale_cam = cam(input_tensor=input_tensor)

    # In this example grayscale_cam has only one image in the batch:
    grayscale_cam = grayscale_cam[0, :]
    # rgb_img = input_tensor.permute(1, 2, 0).numpy()
    rgb_img = torch.reshape(input_tensor, (3,256 ,256)).permute(1,2,0).numpy()
    rgb_img -= rgb_img.min()
    rgb_img /= rgb_img.max()
