...
def get_soft_scores_and_true_labels(dataset, model):
    ...
    # using torch.no_grad() to disable gradient calculation when 
    # we are evaluating the model.
    # This is useful when we are not interested in backpropagating
    # through the model and want to save memory.
    # When we are using a model for evaluation purposes, we typically do not need
    # to compute the gradients of the model's parameters with respect to the loss.
    # Instead, you just want to compute the forward pass of the model and get the predictions.
    # By disabling gradient calculation, we are saving memory since PyTorch will not need to 
    # keep track of the intermediate computations needed to compute the gradients.
    with torch.no_grad():   
        for _, (inputs, targets) in enumerate(dataloader):
            inputs, targets = inputs.to(device) , targets.to(device)
            pred = model(inputs) # outputs two scores: score for how real and how Fake the image is
