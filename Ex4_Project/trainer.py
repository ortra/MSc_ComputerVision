...
class Trainer:
    ...
    def train_one_epoch(self) -> tuple[float, float]:
        ...
        for batch_idx, (inputs, targets) in enumerate(train_dataloader):
            ...
            # references:
            # 1) https://pytorch.org/tutorials/recipes/recipes/zeroing_out_gradients.html
            ...
            # backpropogation: forward + backward + optimize
            pred = self.model(inputs) # outputs two scores: score for how real and how Fake the image is
            ...
            # The classification is done by taking the argmax of the two scores (see figure 3.1 in ass. document)
            correct_labeled_samples += (pred.argmax(1) == targets).type(torch.float).sum().item()
            accuracy = (correct_labeled_samples/num_of_samples)*100

            if batch_idx % print_every == 0 or \
                    batch_idx == len(train_dataloader) - 1:
                print(f'Epoch [{self.epoch:03d}] | Loss: {avg_loss:.3f} | '
                      f'Acc: {accuracy:.2f}[%] '
                      f'({correct_labeled_samples}/{num_of_samples})')
    ...
    def run(self, epochs, logging_parameters: LoggingParameters):
        ...
        for self.epoch in range(1, epochs + 1):
            ...
            if val_acc > best_acc:
                ...
                if (logging_parameters.model_name=='MyNet'):
                    torch.save(state, 'competition_model.pt')
                else:
